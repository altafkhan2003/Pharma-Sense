(() => {
  ...
  function renderStockPage() {
    ...
    if (!rows) return;
    const activeFilter =
      document.querySelector(".stock-tab.active")?.dataset.filter || "all";
    const query =
      document
        .querySelector("[data-stock-search]")
        ?.value.trim()
        .toLowerCase() || "";
    ...
    rows.innerHTML =
      filtered
        .map(
          (item) =>
            `<tr><td><span class="medicine-dot green"></span>${escapeHtml(item.name)}</td><td>${escapeHtml(item.brand)}</td><td>${escapeHtml(item.category)}</td><td>${item.stockQty.toLocaleString("en-IN")}</td><td>${escapeHtml(item.unit)}</td><td>${formatDate(item.expiryDate)}</td><td><span class="stock-status ${statusClass(item.status)}">● ${escapeHtml(item.status)}</span></td><td><button class="row-action" aria-label="Edit ${escapeHtml(item.name)}">✎</button></td></tr>`,
        )
        .join("") ||
    ...
    if (count)
      count.textContent = `Showing ${filtered.length} of ${getInventory().length} medicines`;
  }
  ...
  function setUpStockFilters() {
    ...
    document
      .querySelector("[data-stock-search]")
      ?.addEventListener("input", renderStockPage);
    renderStockPage();
  }
