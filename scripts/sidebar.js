(() => {
  ...
  const navItems = [
    ...
    ["sales", "empty.html?page=sales", "▤", "Sales & Invoicing"],
  ...
  sidebar.innerHTML = `
    ...
    <nav class="navigation" aria-label="Main navigation">
      ${navItems.map((item) => item[0] === "divider"
        ? '<div class="sidebar-divider"></div>'
        : `<a href="${item[1]}" class="nav-item ${activePage === item[0] ? "active" : ""}"><span class="nav-icon">${item[2]}</span>${item[3]}</a>`).join("")}
    </nav>
